...
void ReadingClub::generateCatalog() {
	...
	for(size_t i =0; i < books.size(); i++)
	{
		...
		while(!it.isAtEnd())
		{
			...
			if(it.retrieve().getAuthor() == books.at(i)->getAuthor() && it.retrieve().getTitle() == books.at(i)->getTitle())
			{
				vector<BookCatalogItem>tmp;
				tmp.push_back(it.retrieve());
				tmp[0].addItems(books.at(i));
				cout << tmp[0].getItems().size() << endl;
				catalogItems.remove(it.retrieve());
				catalogItems.insert(tmp[0]);
				flag =1;
			}
		...
		if(flag == 0)
		{
			BookCatalogItem a(books.at(i)->getTitle(),books.at(i)->getAuthor(),books.at(i)->getYear());
			a.addItems(books.at(i));
